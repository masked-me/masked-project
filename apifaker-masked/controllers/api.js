...
const json2excel = require("./yaml2excel");
const word2excel = require("./word2excel");
const { multer } = require('../lib/tools')
 ...
 */
router
  .route("/apis/transform")
  .get(function (req, res) {
    //fill with empty ApiInfo object
    res.render("transform", { apiInfo: models.transformInfoModel });
  })
  .post(function (req, res) {
    //all data is in data field
    var data = req.body.data;
    ...
    try {
      ...
      console.log(e);
      res.json({ retcode: -1, retmsg: req.t("tips.yamlFormatErr") });
      return;
    }
    let downloadUrl = json2excel(data);
    res.json({ retcode: 0, retmsg: req.t("tips.saveSuccess"), downloadUrl });
  });

  router
  .post("/apis/transformWord" ,multer().single('file'),function (req, res) {
    word2excel().then(url=>{
      let downloadUrl = url;
      res.json({ retcode: 0, retmsg: req.t("tips.saveSuccess"), downloadUrl });
      return
      ...
      console.log(e);
      res.json({ retcode: -1, retmsg: req.t("tips.wordFormatErr") });
      return;
    })
  });
 ...
 * View api page
 */
router.route("/apis/view/:id").get(function (req, res) {
  var id = req.params.id;
  db.apiList.findOne({ _id: id, deleted: false }, async function (err, doc) {
    let needMock = true;
    await mockDataPromise(doc.results, doc.demo, needMock).then(
      (proResults) => {
 ...
 * Delete api by id
 */
router.route("/apis/delete/:id").get(function (req, res) {
  var id = req.params.id;
...
async function coverApi(apiArr, req, res) {
  for (let i = 0; i < apiArr.length; i++) {
    var exitApi = isError(
      await dealDatabase.findApi(apiArr[i].apiHead.name),
      req,
      res
    );
    if (exitApi.length > 0) {
    ...
    var newApi = isError(
      await dealDatabase.saveApi({
        ...apiArr[i].apiHead,
        params: apiArr[i].params,
        results: apiArr[i].results,
        demo: apiArr[i].demo,
        createTime: new Date(),
        updateTime: new Date(),
        deleted: false,
    ...
    var newSimulators = isError(
      await dealDatabase.saveSimulators(simulators),
      req,
      res
    );
  }
